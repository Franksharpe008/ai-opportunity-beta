...
const MOBILE_V9='https://live-shift-command-v741-mobile-5xuo8fp2t.vercel.app';
...
const MOBILE_V10_JS=['lsc-v10-core.js','lsc-v10-ai-world.js','lsc-v10-mobile.js','lsc-v10-floor-ops.js','lsc-v10-hourly-performance.js'];
const MANAGER_V10_CSS=['lsc-command-v10.css','lsc-v10-capacity.css','lsc-v10-brief.css'];
const MANAGER_V10_JS=['lsc-v10-core.js','lsc-v10-ai-world.js','lsc-v10-floor-ops.js','lsc-v10-ui.js','lsc-v10-cause-extension.js','lsc-v10-capacity.js','lsc-v10-capacity-ui.js','lsc-v10-brief.js'];
...
function writeProtectedManagerApi(dir){const api=path.join(dir,'api');fs.mkdirSync(api,{recursive:true});fs.writeFileSync(path.join(api,'_proxy.js'),proxyHelper());for(const n of ['state','archive','intelligence'])fs.writeFileSync(path.join(api,`${n}.js`),apiFunction(n,MANAGER_V9,true));writeBase(dir,MANAGER_V9,BASE_MANAGER)}
function writeMobileApi(dir){const api=path.join(dir,'api');fs.mkdirSync(api,{recursive:true});fs.writeFileSync(path.join(api,'_proxy.js'),proxyHelper());for(const n of ['state','archive','intelligence'])fs.writeFileSync(path.join(api,`${n}.js`),apiFunction(n,MANAGER_LIVE,false));writeBase(dir,MOBILE_V9,BASE_MOBILE)}
...
let manager=prepareHtml(await text(MANAGER_LIVE+'/'),{surface:'MANAGER',css:MANAGER_V10_CSS,js:MANAGER_V10_JS});
fs.writeFileSync(path.join(managerOut,'index.html'),manager);for(const n of [...MANAGER_V10_CSS,...MANAGER_V10_JS])copy(n,managerOut);writeProtectedManagerApi(managerOut);writeRuntimeFiles(managerOut);writeProject(managerOut,MANAGER_PROJECT);

let mobile=prepareHtml(await text(MOBILE_LIVE+'/'),{surface:'MOBILE',css:MOBILE_V10_CSS,js:MOBILE_V10_JS});
fs.writeFileSync(path.join(mobileOut,'index.html'),mobile);for(const n of [...MOBILE_V10_CSS,...MOBILE_V10_JS])copy(n,mobileOut);writeMobileApi(mobileOut);writeRuntimeFiles(mobileOut);writeProject(mobileOut,MOBILE_PROJECT);

for(const [label,dir] of [['manager',managerOut],['mobile',mobileOut]])console.log(`${label}: ${dir}`);
console.log('SAFE IN-PLACE TOPOLOGY: manager protects original V9 APIs with OIDC; mobile uses live manager authority; V10 remains additive.');
